...
class Hint extends React.Component<HintProp, HintState> {
  ...
  Hint = () => {
    ...
    useEffect(() => {
      const closeOnClickOutside = (e: MouseEvent) => {
        if (!hintref?.current?.contains(e.target as Node)) this.ClosePopup();
      };
  ...
  HintPopup = () => {
    return (
      <div className={`hint-popup ${this.state.popupVisible ? 'visible' : 'invisible'}`}>
